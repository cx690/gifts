<template>
	<div class="main">
		<header class="header">
			<div class="item">
				检索：
				<a-input-search v-model="form.kw" placeholder="请输入关键字" style="width: 200px" @search="onSearch" />
			</div>
			<div class="item">
				创建时间：
				<a-range-picker v-model="form.time" valueFormat="YYYY-MM-DD HH:mm" :showTime="{
					format:'HH:mm'
		...
		<a-modal :title="current?'修改数据':'新增数据'" :visible="visible" @ok="save" @cancel="visible = false" :confirm-loading="confirmLoading">
			<a-form-model ref="ruleForm" :model="addForm" :rules="rules" :labelCol="{span:4}" :wrapperCol="{span:16}">
				...
				<a-form-model-item prop="price" label="金额">
					<a-input type="number" v-model="addForm.price" placeholder="请输金额" class="input" @blur="onBlur" />
				</a-form-model-item>
				<a-form-model-item prop="desc" label="备注">
					<a-textarea :maxLength="200" v-model="addForm.desc" style="resize:none;width:400px;height:100px" placeholder="备注"
					 class="input" />
				</a-form-model-item>
			</a-form-model>
		</a-modal>
